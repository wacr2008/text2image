...
def add_image_data_into_dset(DB_FNAME,data_path):
  folders=os.listdir(data_path)#这里都和我图片的位置有关
  folders.sort()
  numclass=len(folders)
  index=0 
  i=0
  writer = tf.python_io.TFRecordWriter(DB_FNAME)
...
#   for imname in folders:
#     if imname.endswith('.jpg'):
#       index++  
#       full_path=data_path+imname
#       print(full_path,imname)
      
#       img=Image.open(full_path)
#       imgSize=img.size
#       rawData=img.tobytes()  #将图片转化为原生bytes 

#       example = tf.train.Example(features=tf.train.Features(feature={
#         "label": tf.train.Feature(int64_list=tf.train.Int64List(value=[index])),
#         'img_raw': tf.train.Feature(bytes_list=tf.train.BytesList(value=[rawData]))
#       }))
